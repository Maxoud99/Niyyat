...
- Consume model output by local_id (0/1/2) → map to our global_ids.
- Switchable fallback: PURE_FALLBACK_MINUS_ONE = True forces pure -1 fallback.
...
# Outputs
OUT_LABELS_CSV = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/second-trial-llama/intent_labels.csv"
OUT_EXPL_CSV   = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/second-trial-llama/intent_explanations.csv"
...
# ======= Heuristic fallback parameters (kept; used only if PURE_FALLBACK_MINUS_ONE=False) =======
HEUR_EDU_RANK = {
...
def heuristic_record_score(clean_row: Dict[str,str], manip_row: Dict[str,str], mask_row: Dict[str,str]) -> int:
    ...
    Only used if PURE_FALLBACK_MINUS_ONE is False.
    """
    score = 0
    # education-num
...
def parse_llm_json(text: str) -> Optional[dict]:
    ...
    # 2) If it's JSON-as-a-string (i.e., whole thing is quoted), unescape once
    if len(s) >= 2 and s[0] == '"' and s[-1] == '"':
        try:
            s = json.loads(s)  # unescape inner JSON string
        except Exception:
            pass
        s = (s or "").strip()
...
# -------- consume LLM obj by local_id (robust to bad row_id) --------
