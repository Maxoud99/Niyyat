...
"""
Reco# Processing configuration
CHUNK_SIZE = 5  # Reduced from 7 to decrease input tokens and fit within 4000 limit
MAX_RECORDS = 14  # Set to None for full dataset
DEBUG_MODE = False  # Save LLM responses to debug file (disable for full run)evel Intent Attribution for Twitter Bot Detection via Meta-Llama-3-70B-Instruct — Few-shot
------------------------------------------------------------------------------------
Llama-3 via TGI with few-shot examples for Twitter bot detection:
...
- Llama-3 chat template with proper stop tokens
- Pure fallback: if no parse, set all changed features to -1

Model: Meta-Llama-3-70B-Instruct (via TGI on port 6100)
"""
...
# ======== CONFIG =========
# =========================
SERVER_URL = "http://127.0.0.1:6100/generate"  # Meta-Llama-3-70B-Instruct TGI endpoint
MAX_NEW_TOKENS = 800                           # Reduced to fit within 4000 token limit (was 1200)
...
# Outputs
OUT_DIR = "/home/mohamed/error_injector/llms_baseline/klim-kireev/datasets/twitter-bot/intent-attribution/outputs/few-shots-llama"
...
CHUNK_SIZE = 7  # Reduced to fit within token limit with few-shot examples
...
"""
    ...
    # Combine system role + rubric + rules + few-shot into system prompt
    system_prompt = f"{ROLE_BLOCK}\n{RUBRIC}\n{sys_rules}\n\n{fewshot}"
    
    # Format with Llama-3 chat template
    return llama3_chat(system_prompt, user_content)
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str, max_records: Optional[int] = None):
        ...
        for chunk_idx in range(n_chunks):
            ...
            if DEBUG_MODE:
                ...
                with open(debug_file, "w", encoding="utf-8") as f:
                    ...
                    f.write(f"\n\n=== END RESPONSE (length: {len(llm_response)} chars) ===\n")
                    ...
                    for col in TWITTER_BOT_COLS:
                        if col != "user_id" and mask_rows[i][col] == "1":
                            ...
                            explanations_rows.append({
                                ...
                                "rule": "intent_attribution_few_shot",
                                "details": '{"reason": "llm_json_parse_failure", "source": "fallback"}',
                        ...
                        if col != "user_id" and mask_rows[i][col] == "1":
                            ...
                            if key in decisions:
                                ...
                                explanations_rows.append({
                                    ...
                                    "diagnosis": "0",
                                    "rule": "intent_attribution_few_shot",
                                    "details": '{"reason": "llm_missing_feature_decision", "source": "fallback"}',
                                    "mapping_status": "matched",
                                    "mapping_score": ""
                                })
    ...
    stats = {
        ...
        "endpoint": SERVER_URL,
        "version": "few_shot_enhanced_with_rubric",
    ...
    print("="*80)
    print(f"Model:              Meta-Llama-3-70B-Instruct (Few-Shot Enhanced)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Twitter Bot Intent Attribution Pipeline (Few-Shot Enhanced with Llama)")
    ...
    # Override USE_FEW_SHOT if flag provided
    if args.no_few_shot:
        USE_FEW_SHOT = False
    
    print("="*80)
    print("TWITTER BOT INTENT ATTRIBUTION PIPELINE")
    print("Few-Shot Enhanced with Rubric (Llama-3-70B)")
    print("="*80)
    print(f"Model: Meta-Llama-3-70B-Instruct")
