...
"""
Record-level Intent Attribution via Local LLM (Llama-3-Instruct, via TGI)
-------------------------------------------------------------------------
Surgical fixes:
...
- Leave parsing/fallback/eval structure intact.

Outputs:
- intent_labels-test.csv  (generated mask: GT 1-cells set to ±1, GT 0-cells stay 0)
- intent_explanations-test.csv
"""
...
# ======== CONFIG =========
# =========================
SERVER_URL = "http://127.0.0.1:6000/generate"  # TGI endpoint (Llama-3-Instruct)
MAX_NEW_TOKENS = 256
REQUEST_TIMEOUT_SEC = 120

# File paths (keep yours)
...
# Outputs
OUT_LABELS_CSV = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/second-trial-mixtral/intent_labels.csv"
OUT_EXPL_CSV   = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/second-trial-mixtral/intent_explanations.csv"
...
# ======= Heuristic fallback parameters (balanced, deterministic) =======
HEUR_EDU_RANK = {
...
def heuristic_record_score(clean_row: Dict[str,str], manip_row: Dict[str,str], mask_row: Dict[str,str]) -> int:
    """
    Positive → likely intentional (utility-seeking).
    Negative → likely unintentional (benign human error).
    """
    score = 0
    # education-num
    ...
    for c in ("age","fnlwgt","capital-loss"):
        if mask_row.get(c) == "1":
            ...
            if a is not None and b is not None and abs(b - a) <= SMALL_NUM_DRIFT:
                score -= 1
    # case-only changes → unintentional
...
def _from_decisions_list_obj(obj: dict, global_ids: List[int]) -> Optional[List[Tuple[int,int,str]]]:
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str):
    ...
    for j in tqdm(range(n_clean), desc="Groups (clean + 3 manipulated)", ncols=100):
        ...
        # Build chat-templated input and call TGI
        ...
        # decisions per rid
        decisions: Dict[int, Tuple[int,str,str]] = {}  # rid -> (label, reason, source)

        if parsed is None:
            # Balanced heuristic fallback for all three
            for k, rid in enumerate(global_ids):
                ...
                decisions[rid] = (label, f"heuristic_score={sc}", "fallback")
            sources_hist["fallback"] += 1
        else:
            # Fill from parsed; for any missing entry, use heuristic fallback
            any_from_llm = False
            for k, rid in enumerate(global_ids):
                ...
                if tup is None or tup[1] is None:
                    sc = heuristic_record_score(clean_row, manip_rows[k], mask_rows[k])
                    label = 1 if sc >= FALLBACK_INTENT_THRESHOLD else -1
                    decisions[rid] = (label, f"heuristic_missing_score={sc}", "fallback_missing")
                else:
