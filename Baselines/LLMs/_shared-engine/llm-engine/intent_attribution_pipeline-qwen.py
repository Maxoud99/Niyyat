...
"""
Record-level Intent Attribution via Local LLM (Qwen-2.5 via TGI)
---------------------------------------------------------------
This is a Qwen-2.5 version of the Llama script, adapted for Qwen's chat template
and using the endpoint configured in docker-compose-qwen.yml.

Key changes vs. Llama:
- Chat template: Qwen uses a different format with <|im_start|> and <|im_end|> tokens
- Server endpoint: port 6300 (as per docker-compose-qwen.yml)
- params: do_sample=False, return_full_text=False, stop=["<|im_end|>"]
...
# Outputs
OUT_LABELS_CSV = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/first-trial-qwen/intent_labels.csv"
OUT_EXPL_CSV   = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/first-trial-qwen/intent_explanations.csv"
# Adult schema
ADULT_COLS = [
...
def parse_llm_json(text: str) -> Optional[dict]:
    ...
    for junk in ("</think>", "<|im_end|>", "<|eot_id|>"):
        s = s.replace(junk, "")
    ...
    if isinstance(decs, list) and (len(decs) == 0 or isinstance(decs[0], dict)):
        ...
        for lid in (0, 1, 2):
            ...
            try:
                ...
                dec = None
            # Get reason, with fallback based on which key was used
...
def make_group_prompt(clean_row: Dict[str, str],
    ...
    """
    SAME content as Llama version — strict JSON requested:
    ...
    sys_rules = (
        ...
        '  {"decisions": [{"local_id": 0, "decision": 1 or -1, "reason": "brief explanation"}, ...]}\n'
        "- MUST include 'decision' and 'reason' fields for each of the 3 records.\n"
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str):
    ...
    for j in tqdm(range(n_clean), desc="Groups (clean + 3 manipulated)", ncols=100):
        ...
        # SAME prompt as Llama, just wrapped in Qwen chat template
        prompt_text = make_group_prompt(clean_row, manip_rows, mask_rows, global_ids)
        chat_inputs = qwen_chat(
