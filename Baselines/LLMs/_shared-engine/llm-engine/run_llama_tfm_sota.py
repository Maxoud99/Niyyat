...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Llama-3 Variants — TFM-Inject Adult Income Dataset
==============================================================================

Starts the Llama-3 TGI Docker container, waits for the server to be ready,
then runs three prompt strategies back-to-back on the Mixed SOTA
...
Key differences from the LLM-dataset pipelines:
  ...
  • The Llama TGI server is started via Docker Compose before any queries.

Model: meta-llama/Meta-Llama-3-70B-Instruct served by TGI on port 6100
Template: Llama-3 chat  (<|begin_of_text|><|start_header_id|>...<|eot_id|>)

Usage
-----
  # Quick test on first 100 error records (100 TGI calls per variant)
  python run_llama_mixed_sota.py --n_records 100

  # Full run
  python run_llama_mixed_sota.py

  # Skip server startup if already running
  python run_llama_mixed_sota.py --no_start_server --n_records 200

  # Run only specific variants
  python run_llama_mixed_sota.py --variants zero_shot,info
...
---------
  ...
  --variants         Comma-separated subset to run: zero_shot,info,few_shot
  --no_start_server  Skip Docker Compose startup (server already running)
  --no_stop_server   Skip Docker Compose teardown after run
  --server_url       TGI endpoint (default: http://127.0.0.1:6100/generate)
  --compose_file     Path to docker-compose YAML (default: auto-detected)
  --server_wait      Seconds to wait for server readiness (default: 300)
"""
...
# ── defaults ─────────────────────────────────────────────────────────────────
# TFM data paths
TFM_DIR = "/home/mohamed/error_injector/llms_baseline/tfm_error_injection/output/adult/tabpfn"

DEFAULT_DIRTY = "/home/mohamed/error_injector/llms_baseline/tfm_error_injection/output/adult/tabpfn/dirty.csv"
DEFAULT_CLEAN = "/home/mohamed/error_injector/llms_baseline/error-paper/data/adult_clean.csv"
DEFAULT_MASK  = "/home/mohamed/error_injector/llms_baseline/tfm_error_injection/output/adult/tabpfn/mask.csv"

DEFAULT_SERVER_URL = "http://127.0.0.1:6100/generate"
DEFAULT_HEALTH_URL = "http://127.0.0.1:6100/health"

# Docker-compose file (auto-detected relative to this script)
_SCRIPT_DIR  = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT   = os.path.abspath(os.path.join(_SCRIPT_DIR, "..", "..", "..", ".."))
DEFAULT_COMPOSE_FILE = os.path.join(_REPO_ROOT, "docker-compose", "docker-compose-llama.yml")
...
_EOT = "<|eot_id|>"
_BOT = "<|begin_of_text|>"
_SYS = "<|start_header_id|>system<|end_header_id|>\n"
_USR = "<|start_header_id|>user<|end_header_id|>\n"
_AST = "<|start_header_id|>assistant<|end_header_id|>\n"

# Per-variant token budgets
_TOKENS = {
    "zero_shot": 256,
    "info":       96,
    "few_shot":   96,
...
def start_llama_server(compose_file: str, health_url: str, wait_sec: int = 300) -> None:
    """Start Llama TGI container via Docker Compose and wait until healthy."""
    if not os.path.exists(compose_file):
        sys.exit(f"[ERROR] docker-compose file not found: {compose_file}\n"
                 f"Pass --compose_file <path> to specify it explicitly.")
    print(f"\n[Server] Starting Llama-3 TGI via Docker Compose…")
...
def stop_llama_server(compose_file: str) -> None:
    """Stop Llama TGI container via Docker Compose."""
    if not os.path.exists(compose_file):
        print(f"[WARN] compose file not found, skipping teardown: {compose_file}")
        return
    print(f"\n[Server] Stopping Llama-3 TGI container…")
    result = subprocess.run(
...
def row_to_dict(df: pd.DataFrame, idx: int) -> Dict[str, str]:
    return {c: to_str(df.at[idx, c]) for c in ADULT_FEATURE_COLS}


def blind_mask_row(mask_df: pd.DataFrame, idx: int) -> Dict[str, str]:
    """Returns '1' if cell was changed (|mask| != 0), '0' otherwise — hides intent."""
    return {
        c: ("1" if str(mask_df.at[idx, c]).strip() not in ("0", "0.0", "") else "0")
        for c in ADULT_FEATURE_COLS
    }
...
def llama3_chat(system_text: str, user_text: str) -> str:
    """Single-turn Llama-3 chat template, assistant header left open."""
    return (
        f"{_BOT}"
        f"{_SYS}{system_text}{_EOT}"
        f"{_USR}{user_text}{_EOT}"
        f"{_AST}"
    )
...
def call_tgi(
    ...
    """POST to TGI /generate, return generated_text."""
    payload = {
        "inputs": inputs_str,
        "parameters": {
...
def parse_llm_json(text: str) -> Optional[dict]:
    """Robustly extract first top-level JSON object from model text."""
    if not text:
        return None
    s = text.lstrip("\ufeff").strip()
    for junk in ("```json", "```", "~~~json", "~~~", "</think>", _EOT):
        s = s.replace(junk, "")
    s = s.strip()
...
def extract_decision(obj: Optional[dict], raw: str) -> Optional[int]:
    """
    Extract a single integer decision (1 or -1) from parsed JSON or raw text.
    ...
    Accepts multiple schemas:
      ...
      {"intentional": true/1}
      {"0": 1}   (compact map)
      1 or -1    (bare integer or string)
    """
    # Bare scalar in raw text
...
def prompt_zero_shot(
    ...
    return llama3_chat(_ROLE, user_text)
...
def compute_metrics(y_true: List[int], y_pred: List[int]) -> Dict:
    """Record-level binary metrics: intentional=1, unintentional=-1."""
...
def run_variant(
    ...
    """
    Run one Llama variant over n_records dirty rows (1:1 mixed dataset).
    ...
    # Pre-collect only rows that actually have errors (mask != 0 somewhere)
    error_row_indices = [
    ...
    for i in tqdm(indices_to_run, desc=f"{variant_name}", ncols=90):
        ...
        # Ground-truth: majority sign of non-zero mask cells for this row
        mask_vals = [
            int(mask_df.at[i, c])
            for c in ADULT_FEATURE_COLS
        ...
        # Build chat input
        inputs_str = prompt_fn(clean_row, dirty_row, blind, row_id=i)

        # Call TGI
        t0 = time.perf_counter()
        raw = ""
        try:
            raw = call_tgi(inputs_str, max_new_tokens, server_url)
            status_hist["ok"] += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run three Llama-3 intent-attribution variants on the Mixed SOTA Adult Income dataset."
    )
    ...
    parser.add_argument(
        ...
        help="Number of error records to process (default: ALL). Use 100 for a quick test.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"TGI endpoint (default: {DEFAULT_SERVER_URL})",
    )
    parser.add_argument(
        "--compose_file", default=DEFAULT_COMPOSE_FILE,
        help="Path to docker-compose YAML for Llama",
    )
    parser.add_argument(
        "--server_wait", type=int, default=300,
        help="Seconds to wait for TGI to become ready (default: 300)",
    )
    parser.add_argument(
        "--no_start_server", action="store_true",
        help="Skip Docker Compose startup (server already running)",
    )
    parser.add_argument(
        "--no_stop_server", action="store_true",
        help="Skip Docker Compose teardown after the run",
    )
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(
            _SCRIPT_DIR, "output", f"tfm_sota_llama_{ts}"
        )
    ...
    if not args.no_start_server:
        start_llama_server(args.compose_file, health_url, wait_sec=args.server_wait)
    else:
        print("[Server] Skipping startup (--no_start_server set). Assuming TGI is already running.")
    ...
    for df, name in [(dirty_df, "dirty"), (clean_df, "clean")]:
        missing = [c for c in ADULT_FEATURE_COLS if c not in df.columns]
        if missing:
    ...
    try:
        ...
        if not args.no_stop_server and not args.no_start_server:
            stop_llama_server(args.compose_file)
    ...
    meta = {
        ...
        "model":               "meta-llama/Meta-Llama-3-70B-Instruct (TGI)",
        "server_url":          args.server_url,
