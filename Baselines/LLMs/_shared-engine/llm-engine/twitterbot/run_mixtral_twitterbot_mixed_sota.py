...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Mixtral Variants — Mixed SOTA Twitter Bot Dataset
=============================================================================

Runs three prompt strategies on the Mixed SOTA TwiBot-20 dataset using
Mixtral-8x7B served via TGI on port 6000.
...
Template: Plain text (Mixtral does not use special chat tokens).

Usage
-----
  python run_mixtral_twitterbot_mixed_sota.py --n_records 50
  python run_mixtral_twitterbot_mixed_sota.py
  python run_mixtral_twitterbot_mixed_sota.py --no_start_server --no_stop_server
"""
...
DEFAULT_HEALTH_URL = "http://127.0.0.1:6000/health"
...
def start_server(compose_file: str, health_url: str, wait_sec: int = 600) -> None:
    ...
    print(f"\n[Server] Starting Mixtral TGI…  compose: {compose_file}")
...
# ─────────────────────────────────────────────────────────────────────────────
# Plain-text prompt wrapper (Mixtral has no special chat tokens)
# ─────────────────────────────────────────────────────────────────────────────

def plain_text_prompt(system_text: str, user_text: str) -> str:
...
def make_prompt_zero_shot(clean_rows, dirty_rows, blind_rows, global_ids) -> str:
    ...
    return plain_text_prompt(system, user)
...
def main() -> None:
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(_SCRIPT_DIR, "output", f"twitterbot_mixtral_{ts}")
    ...
    meta = {"run_timestamp": datetime.datetime.now().isoformat(),
            "model": "Mixtral-8x7B (TGI port 6000)",
