...
"""
Record-level Intent Attribution for Twitter Bot Detection via Meta-Llama-3-70B-Instruct
------------------------------------------------------------------------------------------
Adapted from the Gemini bare-minimum pipeline for local LLM (Llama via TGI).
...
- Feed 10 records at a time to the LLM (10 clean + 10 changed)
...
# Outputs
OUT_DIR = "/home/mohamed/error_injector/llms_baseline/klim-kireev/datasets/twitter-bot/intent-attribution/outputs/bare-min-llama"
...
CHUNK_SIZE = 7  # Reduced to fit within 4000 token limit with larger output buffer
...
def make_chunk_prompt(clean_rows: List[Dict[str, str]],
    ...
    Formatted with Llama-3 chat template.
    """
    sys_rules = """Rules:
...
"""
    ...
    # Combine system role + rules into system prompt, user data into user prompt
