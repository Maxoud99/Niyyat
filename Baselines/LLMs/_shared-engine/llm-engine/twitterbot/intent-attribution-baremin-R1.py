...
------------------------------------------------------------------------------------------
Adapted from the bare-minimum Llama pipeline for DeepSeek-R1 (via TGI with Qwen architecture).
...
# Outputs
OUT_DIR = "/home/mohamed/error_injector/llms_baseline/klim-kireev/datasets/twitter-bot/intent-attribution/outputs/bare-min-R1"
...
def make_chunk_prompt(clean_rows: List[Dict[str, str]],
    ...
    """
    Create prompt for a chunk of records.
    
    Returns a prompt asking for FEATURE-LEVEL decisions (not record-level).
    Formatted with Qwen ChatML template (DeepSeek-R1 architecture).
    """
    sys_rules = """Rules:
...
"""
    ...
    # Combine system role + rules into system prompt
