...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Llama Variants — Mixed SOTA Twitter Bot Dataset
==========================================================================

Runs three prompt strategies on the Mixed SOTA TwiBot-20 dataset using
Meta-Llama-3-70B-Instruct served via TGI on port 6100.
...
Template: Llama-3 chat template
...
Usage
-----
  python run_llama_twitterbot_mixed_sota.py --n_records 50
  python run_llama_twitterbot_mixed_sota.py
  python run_llama_twitterbot_mixed_sota.py --no_start_server --no_stop_server
"""
...
def start_server(compose_file: str, health_url: str, wait_sec: int = 600) -> None:
    ...
    print(f"\n[Server] Starting Llama TGI…  compose: {compose_file}")
...
def llama3_chat(system_text: str, user_text: str) -> str:
    return f"{_BOT}{_SYS}{system_text}{_EOT}{_USR}{user_text}{_EOT}{_AST}"
...
def make_prompt_zero_shot(clean_rows, dirty_rows, blind_rows, global_ids) -> str:
    ...
    return llama3_chat(system, user)
...
def main() -> None:
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(_SCRIPT_DIR, "output", f"twitterbot_llama_{ts}")
    ...
    meta = {"run_timestamp": datetime.datetime.now().isoformat(),
            "model": "Meta-Llama-3-70B-Instruct (TGI port 6100)",
