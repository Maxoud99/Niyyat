...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three R1-Qwen Variants — Mixed SOTA Twitter Bot Dataset
=============================================================================

Runs three prompt strategies on the Mixed SOTA TwiBot-20 dataset using
DeepSeek-R1-Distill-Qwen-32B served via TGI on port 6800.

  Variant 1 — zero_shot : plain JSON bundle
...
Key differences from Qwen:
  - JSON seed: assistant turn pre-filled with '{"feature_decisions": ['
    to coax structured output and skip chain-of-thought preamble
  - <think>...</think> blocks are stripped before parsing
  - Stop tokens include </think> and ``` to prevent runaway generation
  - Increased token budget (thinking model uses tokens internally)

Template: Qwen ChatML (<|im_start|> / <|im_end|>) — R1-Distill uses this.

Usage
-----
  python run_r1qwen_twitterbot_mixed_sota.py --n_records 50
  python run_r1qwen_twitterbot_mixed_sota.py
  python run_r1qwen_twitterbot_mixed_sota.py --no_start_server --no_stop_server
"""
...
MAX_NEW_TOKENS      = 1500   # Larger budget — thinking model
REQUEST_TIMEOUT_SEC = 300    # 5 min — R1 thinks before generating

# Qwen ChatML tokens
_IM_START = "<|im_start|>"
_IM_END   = "<|im_end|>"

# JSON seed: opens the feature_decisions list — model continues from here
_JSON_SEED = '{"feature_decisions": ['
...
def start_server(compose_file: str, health_url: str, wait_sec: int = 600) -> None:
    ...
    print(f"\n[Server] Starting DeepSeek-R1-Qwen TGI…  compose: {compose_file}")
...
def stop_server(compose_file: str) -> None:
    ...
    if not os.path.exists(compose_file):
        return
    print(f"\n[Server] Stopping DeepSeek-R1-Qwen TGI…")
...
# ─────────────────────────────────────────────────────────────────────────────
# Qwen ChatML template + JSON seed
# ─────────────────────────────────────────────────────────────────────────────

def qwen_chat_seed(system_text: str, user_text: str, seed: str = _JSON_SEED) -> str:
    """ChatML with assistant turn pre-seeded to skip think preamble."""
    return (
        f"{_IM_START}system\n{system_text}{_IM_END}\n"
        f"{_IM_START}user\n{user_text}{_IM_END}\n"
...
def strip_think(text: str) -> str:
    """Remove <think>...</think> chain-of-thought blocks from R1 output."""
...
# ─────────────────────────────────────────────────────────────────────────────
# JSON parsing (seed reattachment + think stripping)
# ─────────────────────────────────────────────────────────────────────────────

def parse_llm_json(text: str, seed: str = _JSON_SEED) -> Optional[dict]:
    """Reattach JSON seed, strip think blocks, then parse."""
    if not text:
        return None

    # Reattach seed (model continued after it)
    full = seed + text

    # Strip think blocks
    s = strip_think(full)

    for junk in (_IM_END, _IM_START, "```json", "```", "~~~json", "~~~"):
        s = s.replace(junk, "")
    s = s.strip()
    ...
    # Try to extract first balanced JSON fragment
    stack, start = [], None
...
IMPORTANT: Evaluate EACH CHANGED FEATURE INDEPENDENTLY. Be deterministic."""
...
def make_prompt_zero_shot(clean_rows, dirty_rows, blind_rows, global_ids) -> str:
    ...
    return qwen_chat_seed(system, user)
...
def run_variant(variant_name, prompt_fn, server_url, dirty_df, clean_df, mask_df,
    ...
    for chunk_idx in tqdm(range(n_chunks), desc=variant_name, ncols=90):
        ...
        try:
            raw_text = call_tgi(prompt_text, server_url)
            status_hist["ok"] += 1
        except Exception:
            status_hist["http_error"] += 1
        llm_times.append(time.perf_counter() - lt0)

        obj    = parse_llm_json(raw_text, seed=_JSON_SEED)
...
def main() -> None:
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(_SCRIPT_DIR, "output", f"twitterbot_r1qwen_{ts}")
    ...
    meta = {"run_timestamp": datetime.datetime.now().isoformat(),
            "model": "DeepSeek-R1-Distill-Qwen-32B (TGI port 6800)",
