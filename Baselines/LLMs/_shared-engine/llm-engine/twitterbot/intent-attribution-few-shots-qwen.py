...
CHUNK_SIZE = 5  # Reduced to fit within 4000 token limit with few-shot examples
MAX_RECORDS = None  # Set to None for full dataset
DEBUG_MODE = False  # Save LLM responses to debug file (disable for full run)evel Intent Attribution for Twitter Bot Detection via Qwen-2.5 — Few-shot
------------------------------------------------------------------------------------
Qwen-2.5 via TGI with few-shot examples for Twitter bot detection:
...
MAX_NEW_TOKENS = 600                           # Reduced to fit within 4000 token limit with few-shot examples
...
# Outputs
OUT_DIR = "/home/mohamed/error_injector/llms_baseline/klim-kireev/datasets/twitter-bot/intent-attribution/outputs/few-shots-qwen"
...
- statuses_count: 120 → 125
  Rationale: Small activity increase, natural variation → UNINTENTIONAL (-1)

Row ID: 2002  
Changed Features:
- followers_count: 200 → 205
  Rationale: Minor growth, natural variation → UNINTENTIONAL (-1)
  
- description_length: 42 → 45
  Rationale: Small text edit, likely minor update → UNINTENTIONAL (-1)
...
def call_tgi(inputs: str) -> str:
    ...
    try:
        ...
        print(f"[ERROR] TGI call failed: {type(e).__name__}: {str(e)}")
        if DEBUG_MODE:
            import traceback
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str, max_records: Optional[int] = None):
    ...
    print("="*80)
    print(f"Model:              Qwen-2.5 (Few-Shot Enhanced)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Twitter Bot Intent Attribution Pipeline (Few-Shot Enhanced with Qwen)")
    ...
    print("Few-Shot Enhanced with Rubric (Qwen-2.5)")
    print("="*80)
