...
"""
Record-level Intent Attribution for Twitter Bot Detection via DeepSeek-R1 — Few-shot
------------------------------------------------------------------------------------------
DeepSeek-R1 via TGI with few-shot examples for Twitter bot detection:
...
- Qwen ChatML template with proper stop tokens (DeepSeek-R1 uses Qwen architecture)
- JSON seed to coax structured output
- Pure fallback: if no parse, set all changed features to -1

Model: DeepSeek-R1 (via TGI on port 6800, Qwen-based architecture)
"""
...
# Outputs
OUT_DIR = "/home/mohamed/error_injector/llms_baseline/klim-kireev/datasets/twitter-bot/intent-attribution/outputs/few-shots-R1"
...
CHUNK_SIZE = 3  # Reduced from 5 to fit few-shot examples within token limit
...
- statuses_count: 120 → 125
  Rationale: Small count increase, natural activity → UNINTENTIONAL (-1)

Row ID: 2002  
Changed Features:
- followers_count: 1200 → 1215
  Rationale: Minor increase, natural growth → UNINTENTIONAL (-1)
...
"""
    ...
    user_content = (
        ...
        "Analyze each changed feature using the rubric and examples above, then return your feature-level decisions in JSON format now."
    )
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str, max_records: Optional[int] = None):
                    ...
                    for col in TWITTER_BOT_COLS:
                        if col != "user_id" and mask_rows[i][col] == "1":
                            ...
                            explanations_rows.append({
                                ...
                                "rule": "intent_attribution_few_shots",
    ...
    print("="*80)
    print(f"Model:              DeepSeek-R1 (few-shots)")
