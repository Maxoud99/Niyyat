...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Gemini Variants — TFM-Inject Adult Income Dataset
=============================================================================

Runs three Gemini prompt strategies back-to-back on the TFM-Inject
Adult Income dataset (TabPFN-generated errors, 67.3% unintentional / 32.7%
intentional cell ratio):
...
Key differences from the mixed SOTA pipeline variant:
  • The TFM dirty/mask CSVs have 14 columns (no 'class' target column);
    the clean CSV (source) has 15 columns including 'class'.
    'class' is excluded from prompts and metric computation.
  • Errors are generated by TabPFN model re-inference (unintentional) and
    adversarial label-flip conditioning (intentional) — errors are more
    subtle and contextually plausible than rule-based ones.
  • The mask encodes: 1=intentional, -1=unintentional, 0=clean.
  • Ground-truth intent per record = majority sign of non-zero mask cells.

Usage
-----
  # Quick test on first 100 error records (100 API calls per variant)
  python run_gemini_tfm_sota.py --n_records 100

  # Full run (all ~6051 error rows × 3 variants)
  python run_gemini_tfm_sota.py

  # Single variant only
  python run_gemini_tfm_sota.py --variants info --n_records 200

  # Custom paths
  python run_gemini_tfm_sota.py --dirty /path/to/dirty.csv \\
                                 --clean /path/to/adult_clean.csv \\
                                 --mask  /path/to/mask.csv

Arguments
---------
  --dirty       Path to TFM dirty CSV    (default: tfm_error_injection output)
  --clean       Path to clean source CSV (default: error-paper/data/adult_clean.csv)
  --mask        Path to TFM mask CSV     (default: tfm_error_injection output)
  ...
  --variants    Comma-separated subset: zero_shot,info,few_shot (default: all three)
...
# ── defaults ─────────────────────────────────────────────────────────────────
_BASE_ROOT = os.path.join(os.path.dirname(__file__), "..", "..", "..", "..")
_BASE_ROOT = os.path.normpath(_BASE_ROOT)

TFM_OUTPUT = os.path.join(_BASE_ROOT, "tfm_error_injection", "output", "adult", "tabpfn")

DEFAULT_DIRTY = os.path.join(TFM_OUTPUT, "dirty.csv")
DEFAULT_MASK  = os.path.join(TFM_OUTPUT, "mask.csv")
DEFAULT_CLEAN = os.path.join(_BASE_ROOT, "error-paper", "data", "adult_clean.csv")

DEFAULT_MODEL = "gemini-2.5-flash-lite"  # Cost policy 2026-06-26: never gemini-2.5-pro

# 14 feature columns (no 'class' — TFM dirty/mask do not carry the target)
ADULT_FEATURE_COLS = [
...
def compute_diffs(
    ...
    for c in ADULT_FEATURE_COLS:
        if blind.get(c) == "1":
            diffs.append({
                "column": c,
                "from":   clean_row.get(c, ""),
                "to":     dirty_row.get(c, ""),
            })
    return diffs


def ground_truth_intent(mask_df: pd.DataFrame, idx: int) -> int:
    """Majority sign of non-zero mask cells → 1 (intentional) or -1 (unintentional)."""
    vals = [
        int(float(mask_df.at[idx, c]))
        for c in ADULT_FEATURE_COLS
        if str(mask_df.at[idx, c]).strip() not in ("0", "0.0", "")
    ]
    if not vals:
        return -1  # no changes → treat as unintentional (should not happen)
...
def extract_decision(obj: Optional[dict], raw: str) -> Optional[int]:
    ...
    Accepts multiple schemas:
      {"decision": 1}   {"intent": 1}   {"intentional": true/1}
      1  or  -1  (bare integer / string)
    """
...
def run_variant(
    ...
    """
    Run one Gemini variant over n_records error rows from the TFM dataset.

    dirty_df row i corresponds to clean_df row i (same 48,842-row index).
    Ground-truth intent is determined by majority sign of non-zero mask cells.
    """
    ...
    print(f"\n{'='*65}")
    print(f"  VARIANT: {variant_name.upper()}  ({n_records} error records)")
    print(f"{'='*65}")

    # Collect rows that have at least one changed cell
    error_indices = [
        i for i in range(len(dirty_df))
        if (mask_df.iloc[i] != 0).any()
    ]
    indices_to_run = error_indices[:n_records]
    print(f"  (Total rows with errors in dataset: {len(error_indices)})")
    ...
    for i in tqdm(indices_to_run, desc=f"{variant_name}", ncols=90):
        ...
        true_intent = ground_truth_intent(mask_df, i)
        ...
        pred_rows.append({
            ...
            "correct":      int(true_intent == pred_intent),
            "reason":       reason,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run three Gemini intent-attribution variants on the TFM-Inject Adult Income dataset."
    )
    parser.add_argument("--dirty",     default=DEFAULT_DIRTY, help="Path to TFM dirty CSV")
    parser.add_argument("--clean",     default=DEFAULT_CLEAN, help="Path to clean source CSV")
    parser.add_argument("--mask",      default=DEFAULT_MASK,  help="Path to TFM mask CSV")
    parser.add_argument(
        "--n_records", type=int, default=None,
        help="Number of error records to process (default: ALL ~6051). Use 100 for a quick test.",
    )
    parser.add_argument(
    ...
    # Align clean_df to the 14-column feature set (drop 'class' if present)
    for df, name in [(dirty_df, "dirty"), (clean_df, "clean")]:
        missing = [c for c in ADULT_FEATURE_COLS if c not in df.columns]
        if missing:
            sys.exit(f"[{name}] missing feature columns: {missing}")
    ...
    # Align mask columns to feature cols (drop any extra cols like 'class')
    mask_df = mask_df[[c for c in ADULT_FEATURE_COLS if c in mask_df.columns]]
    missing_mask = [c for c in ADULT_FEATURE_COLS if c not in mask_df.columns]
    if missing_mask:
        sys.exit(f"[mask] missing columns: {missing_mask}")
    ...
    print(f"\n{'='*65}")
    print("CROSS-VARIANT COMPARISON — TFM-Inject Adult Income")
    print(f"{'='*65}")
    ...
    meta = {
        ...
        "dataset":             "TFM-Inject Adult Income (TabPFN, v3)",
