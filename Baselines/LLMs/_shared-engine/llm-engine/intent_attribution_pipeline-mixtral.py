...
"""
Record-level Intent Attribution via Local LLM (Mixtral)
------------------------------------------------------
...
Server: Text Generation Inference (TGI) on http://127.0.0.1:6000
"""
...
# ======== CONFIG =========
# =========================
SERVER_URL = "http://127.0.0.1:6000/generate"  # LLMs TGI endpoint
...
# Outputs
OUT_LABELS_CSV = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/first-trial-mixtral/intent_labels.csv"
OUT_EXPL_CSV   = "/home/mohamed/error_injector/llms_baseline/adult_income_dataset/tenth-trial/results/model_outputs/local-llms/first-trial-mixtral/intent_explanations.csv"
    ...
    if isinstance(obj, dict) and isinstance(obj.get("decisions"), list):
        ...
        for it in obj["decisions"]:
            try:
                ...
                rsn = (str(it.get("reason","")).strip() or "reason_not_provided")
                rid = int(rid)
    ...
    if isinstance(obj, dict):
        ...
        for lid in (0,1,2):
            ...
            try:
                ...
                return None
            vals.append((global_ids[lid], dec, "simple_schema"))
        return vals
...
def make_group_prompt(clean_row: Dict[str, str],
    ...
    """
    Strictly request JSON of the form:
    {
      "decisions": [
