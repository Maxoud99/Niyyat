...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Qwen Variants — TFM-Inject Adult Income Dataset
===========================================================================

Starts the Qwen TGI Docker container, waits for the server to be ready,
...
Key differences from the LLM-dataset pipelines:
  ...
  • The Qwen TGI server is started via Docker Compose before any queries.

Model: Qwen/Qwen2.5-32B-Instruct served by TGI on port 6300
Template: Qwen ChatML  (<|im_start|> ... <|im_end|>)
Seed trick: assistant turn opened with '{"0":' for info/few-shot variants
            to force compact JSON continuation.

Usage
-----
  # Quick test on first 100 error records (100 TGI calls per variant)
  python run_qwen_mixed_sota.py --n_records 100

  # Full run
  python run_qwen_mixed_sota.py

  # Skip server startup if already running
  python run_qwen_mixed_sota.py --no_start_server --n_records 200

  # Run only specific variants
  python run_qwen_mixed_sota.py --variants zero_shot,info
...
---------
  ...
  --server_url     TGI endpoint (default: http://127.0.0.1:6300/generate)
...
# JSON seed — opens assistant turn so Qwen continues a compact map
_SEED_MAP = '{"0":'
...
def start_qwen_server(compose_file: str, wait_sec: int = 300) -> None:
    """Start Qwen TGI container via Docker Compose and wait until healthy."""
    if not os.path.exists(compose_file):
        sys.exit(f"[ERROR] docker-compose file not found: {compose_file}\n"
                 f"Pass --compose_file <path> to specify it explicitly.")
    print(f"\n[Server] Starting Qwen TGI via Docker Compose…")
    ...
    _wait_for_server(DEFAULT_HEALTH_URL, wait_sec)


def stop_qwen_server(compose_file: str) -> None:
    """Stop Qwen TGI container via Docker Compose."""
    if not os.path.exists(compose_file):
        print(f"[WARN] compose file not found, skipping teardown: {compose_file}")
        return
    print(f"\n[Server] Stopping Qwen TGI container…")
    result = subprocess.run(
...
def qwen_chat(system_text: str, user_text: str) -> str:
    """Standard Qwen ChatML — no seed (used for zero-shot)."""
    return (
        f"{_IM_START}system\n{system_text}{_IM_END}\n"
        f"{_IM_START}user\n{user_text}{_IM_END}\n"
...
def call_tgi(
    ...
    stop_extra: additional stop tokens (e.g. ["```", "\n\n"] for seeded variants).
    """
    stop_tokens = [_IM_END]
    if stop_extra:
...
# ─────────────────────────────────────────────────────────────────────────────
# Seed handling (info & few-shot variants)
# ─────────────────────────────────────────────────────────────────────────────

def reattach_seed(continuation: str, seed: str = _SEED_MAP) -> str:
    """
    Reattach the JSON seed to continuation, trim everything after first '}'.
    Returns '' if nothing usable.
    """
    s = (continuation or "")
    # strip known junk
    for junk in ("</think>", "```json", "```", _IM_END):
        s = s.replace(junk, "")
...
def parse_llm_json(text: str) -> Optional[dict]:
    ...
    if not text:
        return None
    s = text.lstrip("\ufeff").strip()
    for junk in ("```json", "```", "~~~json", "~~~", "</think>", _IM_END):
        s = s.replace(junk, "")
    s = s.strip()
...
def extract_decision(obj: Optional[dict], raw: str) -> Optional[int]:
    ...
    Accepts multiple schemas:
      ...
      {"intent": 1}
      {"intentional": true/1}
      {"0": 1}  (seeded compact map — key "0" is the single record)
    ...
    if isinstance(obj, dict):
        # Compact-map seed schema: {"0": 1, ...}  or {"0": -1, ...}
        if "0" in obj:
            try:
                v = obj["0"]
...
def run_variant(
    ...
    """
    Run one Qwen variant over n_records dirty rows (1:1 mixed dataset).
    ...
    for i in tqdm(indices_to_run, desc=f"{variant_name}", ncols=90):
        ...
        # Build chat input (returns (inputs_str, uses_seed))
        ...
        # Reconstruct the full JSON text (reattach seed for seeded variants)
        if uses_seed:
            llm_text = reattach_seed(raw_continuation, seed=_SEED_MAP)
        else:
            llm_text = raw_continuation
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run three Qwen intent-attribution variants on the Mixed SOTA Adult Income dataset."
    )
    ...
    parser.add_argument(
        ...
        help="Path to docker-compose YAML for Qwen",
    )
    parser.add_argument(
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(
            _SCRIPT_DIR, "output", f"tfm_sota_qwen_{ts}"
        )
    ...
    if not args.no_start_server:
        start_qwen_server(args.compose_file, wait_sec=args.server_wait)
    else:
    ...
    try:
        ...
        if not args.no_stop_server and not args.no_start_server:
            stop_qwen_server(args.compose_file)
    ...
    meta = {
        ...
        "model":               "Qwen/Qwen2.5-32B-Instruct (TGI)",
        "server_url":          args.server_url,
