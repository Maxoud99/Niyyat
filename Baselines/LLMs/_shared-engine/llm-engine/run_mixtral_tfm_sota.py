...
# -*- coding: utf-8 -*-
"""
Intent Attribution — Three Mixtral Variants — TFM-Inject Adult Income Dataset
==============================================================================

Starts the Mixtral TGI Docker container, waits for the server to be ready,
...
Model : mistralai/Mixtral-8x7B-Instruct-v0.1 served by TGI on port 6000
GPUs  : 0, 1, 2, 3  (4-way)
Template: Llama-3 chat  (<|begin_of_text|>…<|eot_id|>)
         (same template used by the existing Mixtral pipelines after surgical fix)

Usage
-----
  python run_mixtral_mixed_sota.py --n_records 100   # quick test
  python run_mixtral_mixed_sota.py                   # full run
  python run_mixtral_mixed_sota.py --no_start_server # server already up
...
DEFAULT_SERVER_URL = "http://127.0.0.1:6000/generate"

_SCRIPT_DIR  = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT   = os.path.abspath(os.path.join(_SCRIPT_DIR, "..", "..", "..", ".."))
DEFAULT_COMPOSE_FILE = os.path.join(_REPO_ROOT, "docker-compose", "docker-compose-mixtral.yml")
...
# Llama-3 chat-template tokens (same template used by Mixtral pipelines)
_EOT = "<|eot_id|>"
_BOT = "<|begin_of_text|>"
...
def start_mixtral_server(compose_file: str, health_url: str, wait_sec: int = 300) -> None:
    if not os.path.exists(compose_file):
        sys.exit(f"[ERROR] docker-compose file not found: {compose_file}\n"
                 f"Pass --compose_file <path> to specify it explicitly.")
    print(f"\n[Server] Starting Mixtral TGI via Docker Compose…")
    ...
    if result.returncode != 0:
        ...
        print("[Server] Container started (or already running).")
    _wait_for_server(health_url, wait_sec)


def stop_mixtral_server(compose_file: str) -> None:
    if not os.path.exists(compose_file):
        print(f"[WARN] compose file not found, skipping teardown: {compose_file}")
        return
    print(f"\n[Server] Stopping Mixtral TGI container…")
    result = subprocess.run(
...
# ─────────────────────────────────────────────────────────────────────────────
# Llama-3 chat template (used by Mixtral after surgical fix)
# ─────────────────────────────────────────────────────────────────────────────
...
def call_tgi(inputs_str: str, max_new_tokens: int, server_url: str) -> str:
    payload = {
        "inputs": inputs_str,
        "parameters": {
...
def run_variant(
    ...
    for i in tqdm(indices_to_run, desc=f"{variant_name}", ncols=90):
        ...
        if pred_intent is None:
            ...
            reason = str(obj.get("reason", "")).strip() if isinstance(obj, dict) else "scalar"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run three Mixtral intent-attribution variants on the Mixed SOTA Adult Income dataset."
    )
    ...
    parser.add_argument("--n_records", type=int, default=None,
                        help="Error records to process (default: ALL)")
    ...
    if args.out_dir is None:
        ...
        args.out_dir = os.path.join(_SCRIPT_DIR, "output", f"tfm_sota_mixtral_{ts}")
    ...
    if not args.no_start_server:
        start_mixtral_server(args.compose_file, health_url, wait_sec=args.server_wait)
    else:
        print("[Server] Skipping startup (--no_start_server set).")
    ...
    try:
        ...
        if not args.no_stop_server and not args.no_start_server:
            stop_mixtral_server(args.compose_file)
    ...
    meta = {
        ...
        "model":               "mistralai/Mixtral-8x7B-Instruct-v0.1 (TGI)",
        "server_url":          args.server_url,
