...
"""
Record-level Intent Attribution via Google Gemini — BATCHED (Bare-minimum)
------------------------------------------------------------------------------------
Batched processing with BARE-MINIMUM prompt (like basic gemini.py):
- NO rubric, NO diffs, NO few-shot examples
- Processes MULTIPLE groups per API call for efficiency
- Minimal prompt: just clean + manipulated rows with masks
- Same robust parsing as other batched versions
...
# Batching config - AUTO mode: calculates maximum safe batch size
# Set to None for automatic calculation, or specify a number to override
BATCH_SIZE = 10  # Optimal: 10 groups per batch (proven reliable)
...
TOKENS_PER_GROUP = 1250  # Estimated tokens per group (clean + 3 manip + masks + JSON)
...
def call_gemini(model, prompt: str):
    ...
    try:
        ...
        if candidate.finish_reason.value not in (1, 2):
            print(f"[WARNING] Finish reason: {candidate.finish_reason.name}, Safety: {candidate.safety_ratings}")
            return ""
...
def parse_batch_decisions(obj: dict, batch_group_ids: List[List[int]]) -> Optional[Dict[int, Tuple[int, str]]]:
    ...
    Format A (list of groups):
    {
      "groups": [
        {
          "group_id": 0,
          "decisions": [
            {"local_id": 0, "row_id": X, "decision": 1|-1, "reason": "..."},
            ...
          ]
        },
        ...
    ...
    {
      "decisions": [
        {"group_id": 0, "local_id": 0, "row_id": X, "decision": 1|-1, "reason": "..."},
        ...
      ]
    }
    """
...
def make_batch_prompt(batch_data: List[Dict]) -> str:
    """
    Create bare-minimum batched prompt (NO rubric, NO diffs).
    """
    sys_rules = (
        "Rules:\n"
        ...
        '      "group_id": 0,\n'
        '      "decisions": [\n'
        '        {"local_id": 0, "row_id": <global>, "decision": 1|-1, "reason": "<short>"},\n'
        '        {"local_id": 1, "row_id": <global>, "decision": 1|-1, "reason": "<short>"},\n'
        '        {"local_id": 2, "row_id": <global>, "decision": 1|-1, "reason": "<short>"}\n'
        '      ]\n'
        '    },\n'
...
def run_pipeline(manip_path: str, correct_path: str, masks_path: str):
    ...
    if batch_size is None:
        # Calculate maximum batch size that fits within token budget
        system_prompt_tokens = 500  # Estimated tokens for system prompt
        available_tokens = MAX_INPUT_TOKENS - system_prompt_tokens
        calculated_batch_size = int(available_tokens / TOKENS_PER_GROUP)
        batch_size = min(calculated_batch_size, n_clean)  # Don't exceed total groups
        print(f"\n=== AUTO BATCH SIZE CALCULATION ===")
        print(f"Max input tokens: {MAX_INPUT_TOKENS:,}")
        print(f"Estimated tokens per group: {TOKENS_PER_GROUP}")
        print(f"System prompt overhead: {system_prompt_tokens}")
        print(f"Calculated batch size: {batch_size}")
        print(f"This will use {int((n_clean + batch_size - 1) / batch_size)} API call(s) for {n_clean:,} groups")
        print(f"=" * 40)
    ...
    print(f"\n=== Batched Processing (Bare-minimum, ALL DATA IN MEMORY) ===")
    ...
    for batch_idx in tqdm(range(0, n_clean, batch_size), desc="Batches", ncols=100):
        ...
        if isinstance(obj, dict):
            ...
            if batch_decisions is not None:
                ...
                if batch_idx == 0:
                    print(f"[WARNING] First batch parsing failed despite valid JSON")
                if batch_idx == 0:
                    print(f"[DEBUG] parse_batch_decisions returned None!")
                    print(f"[DEBUG] Object structure: {json.dumps(obj, indent=2)[:1000]}")
