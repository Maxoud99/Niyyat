...
"""
Stage 2: LLM-Based Cell-Level Attribution - TabFact Dataset
------------------------------------------------------------
Evaluate Gemini's cell-level error attribution on TabFact erroneous records.
...
- LLM uses clean references + domain knowledge to identify errors
- Save results: cell_suspicions.csv, predicted_masks.csv, explanations.json, metrics.json

Usage:
    python src/detectors/stage2/run_stage2_llm_tabfact.py --chunk-size 5
    python src/detectors/stage2/run_stage2_llm_tabfact.py --chunk-size 5 --num-records 50  # Test on first 50
"""
...
DATASET_PATH = os.path.join(PROJECT_ROOT, "datasets/tabFact/combined_dataset_TRF_no_claim.csv")
MASKS_PATH = os.path.join(PROJECT_ROOT, "datasets/tabFact/error_mask.csv")
OUTPUT_BASE_DIR = os.path.join(PROJECT_ROOT, "outputs/stage2/LLMs/tabFact-dataset")
...
ROLE_BLOCK = (
    "ROLE\n"
    "You are a fact-checking expert analyzing claims from tabular data across multiple domains "
    "(Sports, Business, Politics, Demographics).\n\n"
    ...
    "  - Feature descriptions and domain context\n\n"
    "Your task: Thoroughly examine EVERY cell in EVERY erroneous record to identify ALL suspicious values. "
    "Use the clean references + your domain knowledge and logical reasoning to detect errors."
)
...
def make_batch_prompt(
    ...
    columns : List[str]
        Feature names (excluding is_factual)
    dataset_info : Dict[str, str]
        Dataset context
    ...
    sys_rules = (
        ...
        "1. MINDSET: These records are CONFIRMED ERRONEOUS - at least one error exists in each. Be thorough and precise.\n\n"
        "2. EXAMINATION PROCESS:\n"
        "   a) Compare each erroneous cell to clean reference records\n"
        "   b) Use your domain knowledge about Sports, Business, Politics, Demographics\n"
        "   c) **claim_domain** is 100% error-free - NEVER flag it\n"
        "   d) **subject_type** misclassification alone is NOT an error (e.g., calling a person 'Team' can happen)\n\n"
        "3. FEATURE-SPECIFIC VALIDATION:\n\n"
        "   **subject_entity** (CRITICAL - often contains errors):\n"
        "   - Check for typos in names: missing letters, extra letters, wrong letters\n"
        "   - Compare to clean references - does this entity appear there?\n"
        "   - Verify entity exists in your knowledge base\n"
        "   - Examples: 'miami' instead of 'Miami Heat', 'brown' instead of 'Jim Brown'\n"
        "   - Flag if: Obvious typo, wrong name, entity doesn't exist in domain\n\n"
        "   **subject_location** (BE CAREFUL - many false alarms):\n"
        "   - Only flag if you're 100% certain the location is wrong\n"
        "   - Check for typos: 'the united state' vs 'united states', missing spaces\n"
        "   - Verify entity is actually from this location (use your knowledge)\n"
        "   - Do NOT flag just because it seems unusual - verify first\n"
        "   - Flag if: Clear typo with evidence, factually wrong location with proof\n\n"
        "   **value** (often contains errors):\n"
        "   - Check if numeric value is plausible for context\n"
        "   - Compare to similar values in clean references\n"
        "   - Look for obviously wrong magnitudes (e.g., score of 10000 in baseball)\n"
        "   - Flag if: Impossible value, wrong magnitude, digit transposition\n\n"
        "   **subject_type**:\n"
        "   - Only flag if completely impossible (e.g., 'Athlete' for a company)\n"
        "   - Remember: Misclassification alone is NOT an error\n"
        "   - Flag if: Type makes no sense for the entity at all\n\n"
        "4. SUSPICION SCORING (0.0 to 1.0):\n"
        "   • 0.0-0.2: Perfectly normal - matches clean references, correct value\n"
        "   • 0.3-0.4: Slightly unusual but acceptable\n"
        "   • 0.5-0.6: Moderately suspicious - possible error\n"
        "   • 0.7-0.8: Highly suspicious - likely error with some evidence\n"
        "   • 0.9-1.0: Almost certain - clear factual error with strong evidence\n\n"
        "5. SCORING GUIDELINES:\n"
        "   - For **subject_entity**: Be sensitive - even small typos should score 0.7+\n"
        "   - For **subject_location**: Be conservative - only flag with strong evidence (0.9+)\n"
        "   - For **value**: Check magnitude - wrong order of magnitude should score 0.9+\n"
        "   - For **subject_type**: Very conservative - almost never flag\n\n"
        "6. OUTPUT FORMAT (strict JSON, no extra text):\n"
        '   {"records": [\n'
        '     {"record_id": 0, "cells": [\n'
        '       {"feature": "subject_entity", "value": "ian poulter", "suspicion": 0.1, "reason": "correct name"},\n'
        '       {"feature": "subject_location", "value": "the united state", "suspicion": 0.95, "reason": "typo: missing s, should be united states"},\n'
        '       {"feature": "value", "value": "214", "suspicion": 0.85, "reason": "Ian Poulter score at tournament was 213, not 214"},\n'
        "       ...\n"
        "     ]},\n"
        "     ...\n"
        "   ]}\n\n"
        "7. MANDATORY: Include ALL features for ALL records with their suspicion scores (even if 0.0).\n"
        "   Provide detailed reasoning for any cell with suspicion ≥ 0.5.\n\n"
        "8. BALANCE:\n"
        "   - Don't miss obvious errors in **subject_entity** and **value**\n"
        "   - Don't over-flag **subject_location** unless you're certain\n"
        "   - Use clean references to calibrate what's normal"
    )
...
def evaluate_llm_attribution(
    ...
    """
    print("="*80)
    print("STAGE 2: LLM-BASED CELL ATTRIBUTION - TabFact Dataset")
    print("="*80)
    ...
    clean_records = df[df['is_erroneous'] == 0].drop(columns=['is_erroneous', 'is_factual']).reset_index(drop=True)
    erroneous_records = df[df['is_erroneous'] == 1].drop(columns=['is_erroneous', 'is_factual']).reset_index(drop=True)
    erroneous_original_indices = df[df['is_erroneous'] == 1].index.tolist()
    
    columns = list(erroneous_records.columns)  # Should be: subject_entity, subject_type, subject_location, value, claim_domain
    
    print(f"Total records in dataset: {len(df)}")
    print(f"Clean records: {len(clean_records)}")
    print(f"Erroneous records: {len(erroneous_records)}")
    print(f"Features to analyze: {columns}")
    ...
    # Dataset context - domain knowledge for LLM
    dataset_info = {
        "name": "TabFact Dataset",
        "description": "Fact-checking dataset with claims from tabular data across multiple domains",
        "domains": ["Sports", "Business", "Politics", "Demographics"],
        "features": {
            "subject_entity": "The main entity the claim is about (person, team, company, organization, etc.). Check spelling and existence.",
            "subject_type": "Category of the entity (Athlete, Team, Company, Person, Organization, etc.). Must match the entity.",
            "subject_location": "Geographic location or affiliation (country, city, state, region). Must be correct for the entity.",
            "value": "Numeric value mentioned in the claim (scores, revenues, counts, years, etc.). Must be plausible and accurate.",
            "claim_domain": "Domain category (Sports, Business, Politics, Demographics). Must match the claim content."
        },
        "common_errors": {
            "typos": "Misspelled names, extra/missing characters (e.g., 'tam' vs 'tom', 'the united state' vs 'united states')",
            "wrong_facts": "Incorrect locations, wrong numeric values, misattributed entities",
            "inconsistencies": "Entity type doesn't match entity name, location incorrect for entity",
            "domain_errors": "Wrong domain classification"
        },
        "validation_hints": "Use your knowledge of sports teams, athletes, companies, political figures, and demographics. Check if entities exist, locations are correct, and values are plausible."
    }
    ...
    for chunk_idx in tqdm(range(n_chunks), desc="Processing chunks"):
        ...
        # Build prompt with clean references
    ...
    # Get ground truth masks for erroneous records (exclude is_erroneous column)
    gt_masks = masks_gt.iloc[erroneous_original_indices].drop(columns=['is_erroneous']).reset_index(drop=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Stage 2: LLM-Based Cell-Level Attribution - TabFact Dataset",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of erroneous records to process (default: all)"
    )
